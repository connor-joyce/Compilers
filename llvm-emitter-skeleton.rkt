...
(define (emit-standard-library)  
  ...
  (println "declare i64* @stringCompare(%struct.string* nocapture readonly %str1, %struct.string* nocapture readonly %str2)")
    ...
    (let ([resstr (result->string result)]
      ...
      (cond
        [(not (and (symbol? compsym) (or (string? v1) (Result? v1)) (or (string? v2) (Result? v2))))
         (raise-arguments-error 'emit-bool-comp "compsym should be a symbol and v1 and v2 should be Result (or string) types"
                                "compsym" compsym
                                "v1" v1
                                "v2" v2)]
...
(define (emit-stringcomp compsym v1 v2 [result (make-temp-result)])
  (let ([res (emit-funcall "stringCompare" (make-IntType) `(,v1 ,v2) `(,(make-StringType) ,(make-StringType)))])
        (emit-bool-comp compsym res "0")))
